...
M.setup = function(opts)
	...
	vim.api.nvim_create_autocmd("User", {
		...
		callback = function(args)
			...
			-- paste this inside your MiniFilesBufferCreate callback (replace the old Quick Look mapping)
			vim.keymap.set("n", keymaps.preview_image, function()
				...
				-- prefer mpv: works on Wayland/X11, good keyboard controls
				if exe("mpv") then
					-- start mpv detached and non-blocking, keep window on top without terminal
					-- flags: --no-terminal (no mpv terminal), --force-window=yes ensures a window for images,
					-- --ontop makes it float, --no-border removes decorations (may depend on WM)
					vim.system(
						{ "mpv", "--no-terminal", "--force-window=yes", "--ontop", "--no-border", path },
						{ stdout = false, stderr = false },
						function() end
					)
					vim.notify("Preview: mpv", vim.log.levels.INFO)
					return
				end

				-- fallback: sxiv (keyboard-driven image browser; runs under Xwayland)
				if exe("sxiv") then
					vim.system({ "sxiv", path }, { stdout = false, stderr = false }, function() end)
					vim.notify("Preview: sxiv", vim.log.levels.INFO)
					return
				end

				-- fallback: feh (very lightweight)
				if exe("feh") then
					vim.system({ "feh", path }, { stdout = false, stderr = false }, function() end)
					vim.notify("Preview: feh", vim.log.levels.INFO)
					return
				end

				-- kitty inline (only if using kitty) — shows in-terminal image
				if vim.env.KITTY_WINDOW_ID and exe("kitty") then
					vim.system({ "kitty", "+kitten", "icat", path }, { stdout = false, stderr = false }, function() end)
					vim.notify("Preview: kitty icat", vim.log.levels.INFO)
					return
				end

				-- terminal renderers (open a terminal split and render inside)
				if exe("viu") then
					vim.cmd("belowright split | terminal " .. vim.fn.shellescape(("viu %s"):format(path)))
					return
				end
				if exe("chafa") then
					vim.cmd("belowright split | terminal " .. vim.fn.shellescape(("chafa %s"):format(path)))
					return
				end

				-- final fallback: xdg-open (may open in browser)
				if exe("xdg-open") then
					vim.system({ "xdg-open", path }, { stdout = false, stderr = false }, function() end)
					vim.notify("Preview: xdg-open (fallback)", vim.log.levels.WARN)
					return
				end

				vim.notify("No image viewer found. Install mpv, sxiv, feh, kitty, viu or chafa.", vim.log.levels.ERROR)
			end, { buffer = buf_id, noremap = true, silent = true, desc = "[P] Preview image" })

			-- preview image (Linux-friendly)
			-- vim.keymap.set("n", keymaps.preview_image, function()
			-- 	local mf = mini_files -- assuming you have mini_files local in that scope; otherwise require("mini.files")
			-- 	local curr_entry = mf.get_fs_entry()
			-- 	if not curr_entry or not curr_entry.path then
			-- 		vim.notify("No file selected", vim.log.levels.WARN)
			-- 		return
			-- 	end
			--
			...
			-- 	end
			--
			-- 	-- prefer xdg-open (desktop), then feh/mpv, then terminal renderers (viu/chafa)
			-- 	if exe("xdg-open") then
			-- 		-- open with default desktop application (asynchronously)
			-- 		vim.system({ "xdg-open", path }, { stdout = false, stderr = false }, function() end)
			-- 		vim.notify("Opened with xdg-open", vim.log.levels.INFO)
			-- 		return
			-- 	end
			--
			-- 	if exe("feh") then
			...
			-- 		vim.system({ "feh", path }, { stdout = false, stderr = false }, function() end)
			-- 		vim.notify("Opened with feh", vim.log.levels.INFO)
			-- 		return
			-- 	end
			--
			-- 	if exe("mpv") then
			-- 		-- mpv works well for images and videos; --no-terminal avoids spawning a terminal window
			-- 		vim.system({ "mpv", "--no-terminal", path }, { stdout = false, stderr = false }, function() end)
			-- 		vim.notify("Opened with mpv", vim.log.levels.INFO)
			-- 		return
			-- 	end
			--
			-- 	-- If running in kitty terminal, kitty's icat can show images inline
			-- 	if vim.env.KITTY_WINDOW_ID and exe("kitty") then
			-- 		-- kitty +kitten icat prints image inline in the current kitty window
			-- 		vim.system({ "kitty", "+kitten", "icat", path }, { stdout = false, stderr = false }, function() end)
			-- 		vim.notify("Displayed with kitty icat", vim.log.levels.INFO)
			-- 		return
			-- 	end
			--
			-- 	-- terminal image renderers: open a terminal split with viu or chafa
			-- 	if exe("viu") then
			-- 		-- open a terminal split and run viu on the file
			-- 		vim.cmd("belowright split | terminal " .. vim.fn.shellescape(("viu %s"):format(path)))
			-- 		return
			-- 	end
			--
			...
			-- 	vim.notify(
			-- 		"No supported image viewer found (install xdg-open / feh / mpv / viu / chafa). File: " .. path,
			-- 		vim.log.levels.WARN
			-- 	)
			-- end, { buffer = buf_id, noremap = true, silent = true, desc = "[P] Preview image (Linux)" })
